...
Scopes: `full`, `micron`, `industry`, `quote`, `ecosystem`, `source_audit`, `company:<id>`, `discovery`, `news`, `calendar`, `technology`, `technology:facilities`, `technology:processes`, `technology:products`, `batch`, `maintenance`. Routine work starts with `schedule` and targeted discovery; `full` is an explicit complete source audit. See [cadence and news](CADENCE.md) for targets and event handling, and [manufacturing and technology](TECHNOLOGY.md) for technical fields, source coverage and report review gates. The command creates:
