...
2. 按 `docs/CADENCE.md` 先查新：周日、周三覆盖行业与动态；公司财报只处理已到核查时间的确认日程、未处理的新披露或用户指定范围。周日另按 `calendar_targets` 补查未确认的下一财报日期，不遍历所有公司的财报目录或重读旧报告。发现新财报、修订、临时指引或其他实质披露后，再读取相关原文并更新指标。
3. 核查 TrendForce 等行业原始报告；媒体转引与公开摘要必须保留标记，不得声称已经读取付费全文或原始表格。
   公司范围由 `catalog.monitoring.targets` 注册；定向披露核查与财报日历维护分别取证。新闻巡检发现临时指引、预告、重列或更正时，立即为相关公司建立定向 discovery，并处理披露。日历查询不推进公司财报查新或原文核查时间。无新报告时保留原财季、发布日期与财报核查日。
...
- 核查用 UTC；市场和活动用 ET；原报告的日期按原文。财报不因报告或查新日期变旧而报警，只提示定向核查失败、新披露待处理和已到期的确认日程。行业与动态单独记录，阈值来自 catalog。
- 行情按最近已完成交易日判断，排除已确认的休市日。按钮只载入已发布记录，不触发研究。
...
制造与技术的长期记录位于 `technology.items`；执行规范见 `docs/TECHNOLOGY.md`。周三／周日必须使用 schedule 返回的三类 `technology:*` 查新目标；美光、三星、SK 海力士新财报还需完成 `technology_reviews` 中的逐项结果。印度封测量、晶圆制造产能、1γ 位元密度与公司供给增量分别记录；DDR5/HBM 对照保留产品阶段、容量单位与功耗基准。旧工厂/技术行已通过 catalog replacement 转入长期记录，新闻历史引用保留。
