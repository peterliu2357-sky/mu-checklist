...
2. 按 `docs/CADENCE.md` 先查新：周日覆盖所有公司、行业与动态，周三覆盖行业与动态；已确认财报后安排定向核查，也支持用户按需更新。仅发现新财报、修订、临时指引或其他实质披露后，才读取相关原文并更新指标。
3. 核查 TrendForce 等行业原始报告；媒体转引与公开摘要必须保留标记，不得声称已经读取付费全文或原始表格。
   公司查新覆盖 `catalog.monitoring.targets`，区分公告目录读取与财报原文复核。无新报告时保留原财季、发布日期与财报核查日，只推进有读取证据的查新日期；季度中途修订的指引也应处理。
...
- 核查用 UTC；市场和活动用 ET；原报告的日期按原文。财报不因超过 36 小时而报警，按公司查新、新披露待处理和已确认日程分别提示。行业与动态单独记录，阈值来自 catalog。
