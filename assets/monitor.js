...
(function () {
  ...
  function render(fallback=false) {
    ...
    for(const category of ['business','industry']) document.getElementById(`panel-${category}`).innerHTML=panelWarnings(category)+`<h2>${category==='business'?'公司数据':'行业数据'}</h2><p class="section-intro">${category==='business'?'财务金额为亿美元；FQ 为美光财季。各项附来源和原文位置。':'行业估计、报价与预测分别标识；用于观察终端消耗的间接指标单独说明。'}</p>${data.metrics.map((m,i)=>m.category===category?metricCard(m,i):'').join('')}`;
