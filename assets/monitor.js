...
(function () {
  ...
  function updates() {
    return `<section class="card update-status"><div class="line"><span>最近成功核查</span><strong>${date(data.last_successful_check_at,true)}</strong></div><div class="line"><span>财报覆盖期间</span><strong>${esc(data.financial_period)}<br><span>截至 ${esc(data.financial_as_of)}</span></strong></div><div class="line"><span>收盘价日期</span><strong>${date(data.quote.as_of)}</strong></div><div class="line"><span>持续核查</span><strong>${data.automation.enabled?'每日核查已启用':'尚未启用'}</strong></div><p>${esc(data.automation.note)} 核查日不等于原始数据发布日期。超过 36 小时未完成核查，页面会提示时效风险。所有时间显示为美国东部时间。</p><button class="button" id="refresh-data" style="margin-top:14px">载入最新记录</button><p id="refresh-state" role="status"></p></section><div class="section-heading"><h2>这次改变了什么</h2></div><ol class="timeline">${data.changes.map(c=>`<li><time>${esc(c.date)}</time><h3>${esc(c.title)}</h3><p>${esc(c.text)}</p></li>`).join('')}</ol><div class="section-heading"><h2>核查记录</h2></div><ol class="timeline">${data.check_log.slice(0,7).map(c=>`<li><time>${date(c.at,true)} · ${c.status==='success'?'核查完成':c.status==='failed'?'核查未完成':'部分完成'}</time><p>${esc(c.text)}</p></li>`).join('')}</ol><div class="section-heading"><h2>数据来源</h2><span class="small">点开原始资料</span></div><div class="source-directory">${Object.entries(data.sources).map(([id,s])=>`<div class="source-entry">${sourceLink(id,true)}<p>${esc(s.type)} · 发布 ${esc(s.published_at || '资料页，未标注日期')} · 核查 ${esc(s.checked_at)}</p></div>`).join('')}</div><div class="section-heading"><h2>口径说明</h2></div><ul class="method">${data.methodology.map(s=>`<li>${esc(s)}</li>`).join('')}</ul>`;
  }
  ...
  function render(fallback = false) {
    document.getElementById('checked-stamp').textContent=`核查 ${date(data.last_successful_check_at)} · 财报 ${data.financial_period}`;
    const warnings = freshness(data);
    if(fallback) warnings.unshift(`未能载入最新记录，正显示 ${date(data.updated_at,true)} 保存的备用快照。`);
  ...
  async function load(refresh = false) {
    ...
    try {
      ...
      data=next;render();
      if(refresh) document.getElementById('refresh-state').textContent=`已载入。最新核查：${date(data.last_successful_check_at,true)}。此按钮不触发新一轮研究。`;
    } catch(error) {
      if(!data) {data=JSON.parse(document.getElementById('fallback-data').textContent);render(true);}
      else {document.getElementById('freshness').innerHTML=`<div class="banner" role="alert">最新记录载入失败。保留 ${date(data.updated_at,true)} 的已载入资料。</div>`;if(feedback)feedback.textContent='载入失败，请稍后重试。';}
    }
  }
