(function(){
  ...
  function states(data,now=Date.now()){
    ...
    const topicLabels={'technology:facilities':'工厂与产能','technology:processes':'制程换代','technology:products':'同类产品'};
    for(const key of ['industry','news',...(data.technology?Object.keys(topicLabels):[])]){
      const check=checks.find(c=>c.key===key),label=topicLabels[key]||(key==='industry'?'行业资料':'AI 动态');
      ...
      if(check?.finding==='new_disclosure'&&!check.processed_at)item.warning=`${label}发现新披露，待核查录入。`;
      else if(['failed','pending'].includes(check?.status))item.warning=`${label}最近一次查新未完成，保留已保存资料。`;
