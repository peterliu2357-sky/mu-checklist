(function(){
  'use strict';
  const day=86400000;
  const defaults={financial_discovery_days:7,industry_discovery_days:4,grace_hours:24,quote_stale_sessions:2};
  ...
  function states(data,now=Date.now()){
    ...
    for(const c of companies){
      ...
      else if(event&&now>Date.parse(event.review_after)&&(!check?.checked_at||Date.parse(check.checked_at)<Date.parse(event.scheduled_at)))item.warning=`${c.name}已到预计披露时间，待确认新财报。`;
      else if(Number.isFinite(Date.parse(checked))&&now-Date.parse(checked)>(policy.financial_discovery_days*24+policy.grace_hours)*3600000)item.warning=`${c.name}公告查新已逾期；财报仍为所标注期间。`;
      result.push(item);
    }
